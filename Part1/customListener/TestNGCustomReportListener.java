...
public class TestNGCustomReportListener implements IReporter{
	...
	protected PrintWriter createWriter(String outdir) throws IOException {
		...
		String f = System.getProperty("user.dir")+"\\Reports\\" + df.format(new Date());
		new File(f).mkdirs();
		return new PrintWriter(new BufferedWriter(new FileWriter(new File(f, reportFileName))));
	}
	...
	private void resultSummary(ISuite suite, IResultMap tests, String testname,
		...
		if (tests.getAllResults().size() > 0) {
						...
						+ (description != null && description.length() > 0 ? "("
								...
								+ (null == testInstanceName ? "" : "<br>("
										...
										+ "<td class=numi" + style +"=text-align:left;padding-right:2em >" + firstLine +"<br/></td>"
										 + "<td class=numi" + style + "=text-align:left;padding-right:2em >" + formatter.format(calendar.getTime()) + "<br/></td>" +
										 "<td class='numi' >"
			...
			if (mq > 0) {
				cq += 1;
				writer.print("<tr" + style + (cq % 2 == 0 ? "even" : "odd") + ">" + "<td");
				if (mq > 1) {
					writer.print(" rowspan=" + mq + "");
				}
				writer.println(">" + lastClassName + "</td>" + buff);
	...
	private void startResultSummaryTable(String style) {
		...
		writer.println("<tr><th>Class</th>"
				+ "<th>Scenario</th><th>Exception Info</th><th>Start Time </th><th>Execution Time<br/>(hh:mm:ss)</th></tr>");
		m_row = 0;
	}
...
public void generateSuiteSummaryReport(List<ISuite> suites) {
	...
	for (ISuite suite : suites) {
		...
		for (ISuiteResult r : tests.values()) {
			...
			qty_pass_m += q;
			summaryCell(q, Integer.MAX_VALUE,"pass");
			q = getMethodSet(overview.getSkippedTests(), suite).size();
			qty_skip += q;
			summaryCell(q, 0,"");
			q = getMethodSet(overview.getFailedTests(), suite).size();
			qty_fail += q;
			summaryCell(q, 0,"fail");
...
private void summaryCell(String v, boolean isgood,String value) {
	if(value == "fail"){
	writer.print("<td style='background:indianred' class='numi' + (isgood ? '' : '_attn') + ''>" + v
			+ "</td>");
	}else if(value == "pass"){
		writer.print("<td style='background:limegreen' class='numi' + (isgood ? '' : '_attn') + ''>" + v
				+ "</td>");
	}else{
...
private void summaryCell(int v, int maxexpected,String value) {
	summaryCell(String.valueOf(v), v <= maxexpected,value);
}
...
private void titleRow(String label, int cq, String id) {
	...
	writer.println("><th colspan='10' + cq + ''>" + label + "</th></tr>");
	m_row = 0;
}
